...
class Mod_JUX_MegaMenu {
					...
					if ($modules && count($modules)>0) {
						...
						for ($col=0;$col<$cols;$col++) {
							...
							while ($i<$total) {
								...
								$i += $cols;
								// upgrade to joomla 3
								$mod_params	= $this->createParameterObject($mod->params);
	...
	function genMenuItem($item, $level = 0, $pos = '', $ret = 0) {
		...
		// since 2.1.2 for HTML tooltip
		$tooltip_type	= $tmp->params->get('mega_tooltip', 0);
		if ($tooltip_type) { // HTML tooltip
			$hasTip	= ' hasTip';
			//$title	= $tmp->params->get('mega_tooltip_content', '');
			$title	= '::'.htmlspecialchars($tmp->params->get('mega_tooltip_content', ''), ENT_COMPAT, 'UTF-8', false);
		} else {
			$hasTip	= '';
		...
		if ($active) $active = " class=\"$active $hasTip\"";
...
//				$txt = '<span class="menu-image" style="background-image:url('.$item->menu_image.')"><span class="menu-title">'.$tmpname.'</span></span>' :
//				$txt = '<span class="menu-image no-title"><img src="'.$item->menu_image.'" alt="'.$tmpname.'" /></span>';
				$txt = '<img src="'.$item->menu_image.'" alt="'.$tmpname.'" /><span class="image-title">'.$tmpname.'</span> ' :
				$txt = '<img src="'.$item->menu_image.'" alt="'.$tmpname.'" />';
		} else {
				...
				if ($tmp->url != null)
				{
					switch ($tmp->browserNav)
					{
						...
						case 0:
							...
							// _top
							$data = '<a href="'.$tmp->url.'" '.$active.' '.$id.' '.$title.'>'.$txt.'</a>'/*.'<span  '.$active.' '.$title.'>'.'open'.'</span>'*/;
							break;
						case 1:
							// _blank
		...
		if ($this->getParam ('megamenu')) {
			...
			if ($tmp->megaparams->get('group') && $data) {
				$cls = in_array($item->id, $this->open) ? "group-title active" : "group-title";
				$data = "<div class=\"$cls\">$data</div>";
			}
	...
	function beginSubMenuItems($pid=0, $level=0, $pos, $i, $return = false){
		...
		if (isset ($this->items[$pid]) && $level) {
			...
			if ($this->items[$pid]->megaparams->get('group') && $cols < 2) {
				...
				if (!$colw) $colw = $this->items[$pid]->megaparams->get('colw');
				if(is_null($colw) || !is_numeric($colw))
					$colw	= $this->getParam ('mega-colwidth',200);
					//$colw = 200;
				$style = $colw?" style=\"width: {$colw}px;\"":"";
				$data .= "<div class=\"megacol column".($i+1).($pos?" $pos":"")."\"$style>";
			}
		}
//		// Add a toggle elment for CSS3 style
//		if ($level == 0) {
//			$data	.= "\n\t".'<div id="css3-ToggleResponsive" class="megaMenuToggle">'.JText::_('MOD_JUX_MEGAMENU_TOGGLE_MENU').'<span class="megaMenuToggle-icon"></span></div>';
//		}
	...
	function beginSubMenuModules($item, $level=0, $pos, $i, $return = false){
		...
		if ($level) {
			if ($item->megaparams->get('group')) {
				...
				if (!$colw) $colw = $item->megaparams->get('colw');
				if(is_null($colw) || !is_numeric($colw))
					$colw	= $this->getParam ('mega-colwidth',200);
					//$colw = 200;
				$data .= "<div class=\"megacol column".($i+1).($pos?" $pos":"")."\"$style>";
...
//		if (!preg_match ('/group/', $cls)) $cls .= ($active?" active":"");
	...
	function beginMenuItem($mitem=null, $level = 0, $pos = ''){
		...
		$active	.= ' submenu-align-'. $mitem->params->get('submenu_align', 'auto');
		if ($active) $active = " class=\"$active\"";
		echo "<li $active>";
		if ($mitem->megaparams->get('group')) echo "<div class=\"group clearfix\">";
	}
